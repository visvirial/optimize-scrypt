...
__constant static const uint4 input[] = {
	{0x01000000, 0x00000000, 0x00000000, 0x00000000},
	{0x00000000, 0x00000000, 0x00000000, 0x00000000},
	{0x00000000, 0xa64bac07, 0xfe31877f, 0x31d03252},
	{0x953b3c32, 0x398933af, 0x7a724119, 0xbc4d6fa4},
	{0xa805e435, 0xf083c252, 0xf0ff0f1e, 0x00000000},
};
...
void scrypt_core(uint4 X[8], __global void*restrict scratchpad);
__attribute__((reqd_work_group_size(LOCAL_WORK_SIZE, 1, 1)))
__kernel void run(__global uint8*restrict output, __global void*restrict scratchpad) {
	uint gid = get_global_id(0);
	const size_t offset = gid - get_global_offset(0);
	uint4 X[8];
	uint4 tstate0, tstate1, ostate0, ostate1, tmp0, tmp1, pad0, pad1;
	uint4 data = (uint4)(input[4].x,input[4].y,input[4].z,EndianSwap(gid));
	SHA256_fresh(&pad0, &pad1, input[0], input[1], input[2], input[3]);
	...
	output[offset].s0123 = EndianSwap(ostate0);
	output[offset].s4567 = EndianSwap(ostate1);
}
